...
# 消融实验：w/o NLM
# 定义：保留四层预测、四层 loss、跨层 gate 和残差式门控，只把每层的 NonLocalMask 换成普通卷积预测头。
# 由于这组 decoder ablation 不带对比学习，plain head 仍然返回 (mask, x) 以保持接口一致即可。当前 full decoder 里四个 getmask* 都是 NonLocalMask，这份 patch 就是把它们统一替换掉。
"""
...
class PlainMaskHead(nn.Module):
    """Plain mask prediction head for w/o NLM ablation."""

    def __init__(self, in_channels: int):
        super(PlainMaskHead, self).__init__()
        self.getmask = nn.Sequential(
            nn.Conv2d(in_channels, 16, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(16, 1, kernel_size=3, padding=1)
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        # keep the same (mask, z) interface as NonLocalMask
        # so the rest of the decoder code does not need to change
        mask = self.getmask(x)
        z = x
        return mask, z
...
class ProgressiveContrastiveHead(BaseDecodeHead):
    ...
    def __init__(self, crop_size=(512, 512), reduce_scales=[4, 2, 2, 1], 
        ...
        # w/o NLM: replace NonLocalMask with plain conv heads
        self.getmask4 = PlainMaskHead(self.in_channels[3])
        self.getmask3 = PlainMaskHead(self.in_channels[2])
        self.getmask2 = PlainMaskHead(self.in_channels[1])
        self.getmask1 = PlainMaskHead(self.in_channels[0])
    ...
    def forward(self, inputs: List[torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        if self.training:
            return {'mask1': mask1, 'mask2': mask2, 'mask3': mask3, 'mask4': mask4}
        return {'mask1': mask1}
