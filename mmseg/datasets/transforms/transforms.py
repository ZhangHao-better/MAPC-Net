...
class RandomCropWithExtra(BaseTransform):
    ...
    def __init__(self,
                 ...
                 cat_max_ratio: float = 1.,
                 ignore_index: int = 255,
                 ensure_fg_prob=0.0, ensure_fg_min_pixels=1, ensure_fg_max_retry=20,
                 ensure_fg_min_ratio=0.0, ensure_fg_select_best=False):
        super().__init__()
        assert isinstance(crop_size, int) or (
            isinstance(crop_size, tuple) and len(crop_size) == 2
        ), 'The expected crop_size is an integer, or a tuple containing two '
        ...
        self.ensure_fg_prob = ensure_fg_prob # ensure_fg_prob: 有多大概率强制采到前景（建议 0.5 起步）
        self.ensure_fg_min_pixels = ensure_fg_min_pixels # 前景像素至少多少（建议 256 或 512 起步，512×512 crop 下很合理）
        self.ensure_fg_max_retry = ensure_fg_max_retry # 最多重试多少次（避免死循环）
        self.ensure_fg_min_ratio = ensure_fg_min_ratio # 前景像素至少占比多少（建议 0.01 起步）
        self.ensure_fg_select_best = ensure_fg_select_best # 重试时是否选最优
    ...
    def crop_bbox(self, results: dict) -> tuple:
        ...
        # gt 全 0 短路：整张图都没前景就别 ensure_fg/cat_max_ratio 重试了
        ...
        # 先做 cat_max_ratio（如果你 cat_max_ratio=1.0，这段不会生效）
        if self.cat_max_ratio < 1.:
            for _ in range(10):
                seg_temp = self.crop(results['gt_seg_map'], crop_bbox)
                labels, cnt = np.unique(seg_temp, return_counts=True)
        ...
        # ensure_fg：放到最后，避免被 cat_max_ratio “重采没前景”
        if self.ensure_fg_prob > 0 and np.random.rand() < self.ensure_fg_prob:
            # 门槛：min(固定阈值, 全图fg*ratio)，保证小目标图也“可达”
            min_required = int(self.ensure_fg_min_pixels)
            if getattr(self, 'ensure_fg_min_ratio', 0.0) and self.ensure_fg_min_ratio > 0:
                min_required = min(min_required, max(1, int(fg_full_pixels * float(self.ensure_fg_min_ratio))))
