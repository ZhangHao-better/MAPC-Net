...
class RandomCropWithExtra(BaseTransform):
    ...
    def crop_bbox(self, results: dict) -> tuple:
        ...
        if int(((seg_full > 0) & valid_full).sum()) == 0:
            return crop_bbox
        
        if self.ensure_fg_prob > 0 and np.random.rand() < self.ensure_fg_prob: # 强制采到前景
            for _ in range(self.ensure_fg_max_retry):
                seg_temp = self.crop(results['gt_seg_map'], crop_bbox)
                valid = (seg_temp != self.ignore_index)
                fg = (seg_temp > 0) & valid
                if int(fg.sum()) >= self.ensure_fg_min_pixels:
                    break
                crop_bbox = generate_crop_bbox(img)
