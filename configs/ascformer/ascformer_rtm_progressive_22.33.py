...
model = dict(
    ...
    decode_head=dict(
        type='ProgressiveContrastiveHead',  # Progressive decoder with PSCC-Net architecture
        ...
        # ===== Contrastive hyper-params (from-scratch, balanced but not too conservative) =====
        contrastive_weight=0.01,            # 0.02 太容易把总 loss“抢走”
        contrastive_warmup_iters=20000,     # 从零训：先让 seg 稳住，再逐步加对比
        contrastive_levels=('z1',),     # 先两层，避免 3 层强度过大
        contrastive_proj_dim=128,
        contrastive_samples_per_img=160,    # 每图：pos 160 + neg 160 => 320 points
        contrastive_boundary_ratio=0.25,    # 边界信号稀疏，0.5 太偏边界
     ),
